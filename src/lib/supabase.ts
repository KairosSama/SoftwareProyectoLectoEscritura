...
export function __createSupabaseInternal(opts: {
      ...
      const builder: any = {
        ...
        insert: () => ({
          select: () => ({
            single: () =>
              Promise.resolve({
                data: { id: 'inserted-id', created_at: new Date().toISOString() },
                error: null,
              }),
          }),
        }),
        then: (resolve: any, reject: any) =>
          Promise.resolve({ data: builder._data, error: null }).then(resolve, reject),
      };
      return builder;
    };
    return {
      auth: {
        ...
        signUp: async (_data: any) => ({ data: { user: { id: 'test-user' } }, error: null }),
  ...
  if (!opts.supabaseUrl || !opts.supabaseAnonKey) {
    if (opts.strict) {
      throw new Error(
        'Faltan variables VITE_SUPABASE_URL o VITE_SUPABASE_ANON_KEY (STRICT_SUPABASE_ENV)'
      );
    }
    // eslint-disable-next-line no-console
    console.warn(
      '[supabase] Variables de entorno ausentes, usando mock inofensivo. Configure VITE_SUPABASE_URL / VITE_SUPABASE_ANON_KEY.'
    );

    // 🔧 Fallback “seguro” compatible con API v2: incluye getSession, onAuthStateChange, signOut, etc.
    const mock = {
      auth: {
        ...
        signInWithPassword: async (_creds: any) => ({
          ...
          error: new Error('Mock: sin Supabase real'),
        }),
        signUp: async (_data: any) => ({ data: { user: { id: 'fallback-user' } }, error: null }),
        resetPasswordForEmail: async (_email: string) => ({ data: {}, error: null }),
      },
      from: (_table: string) => ({
        select: () => ({
...
if (isTest) {
  ...
  // @ts-ignore
  const strict =
    typeof process !== 'undefined' && process.env && process.env.STRICT_SUPABASE_ENV === 'true';
  supabase = __createSupabaseInternal({
    isTest: false,
    supabaseUrl,
    supabaseAnonKey,
    strict,
